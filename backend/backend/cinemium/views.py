...
def getUsersByCinemaId(request, cinema_id):
    users = User.objects.raw('SELECT * FROM cinemium.auth_user INNER JOIN ticket ON ticket.user_id = auth_user.id INNER JOIN cinemium.show ON cinemium.show.show_id = ticket.show_id INNER JOIN cinema_hall ON cinema_hall.cinema_hall_id = cinemium.show.cinema_hall_id INNER JOIN cinema ON cinema.cinema_id = cinema_hall.cinema_id WHERE cinema.cinema_id = %s', [cinema_id])
